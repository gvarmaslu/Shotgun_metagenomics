...
binning_maxbin(){
   ...
   do
      ...
      jgi_summarize_bam_contig_depths \
      --outputDepth ${ANALYSIS_FOLDER}/coverage/$(basename ${i%.1*})_depth.txt \
      --pairedContigs ${ANALYSIS_FOLDER}/coverage/$(basename ${i%.1*})_paired.txt \
      --minContigLength 1000 \
      --minContigDepth 2 \
      ${ANALYSIS_FOLDER}/coverage/bam/$(basename ${i%.1*}).sorted.bam

      tail -n+2 ${ANALYSIS_FOLDER}/coverage/$(basename ${i%.1*})_depth.txt | cut -f 1,3 > ${ANALYSIS_FOLDER}/coverage/$(basename ${i%.1*})_maxbin.cov
      ...
      -abund ${ANALYSIS_FOLDER}/coverage/$(basename ${i%.1*})_maxbin.cov \
...
binning_metabat(){
   ...
   do
     # ${TOOLS_FOLDER}/metabat/metabat \
      ...
      -a ${ANALYSIS_FOLDER}/coverage/$(basename ${i%.1*})_depth.txt \
