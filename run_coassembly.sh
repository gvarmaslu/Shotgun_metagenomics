...
run_coassem_spades(){
   ...
   for i in ${finallist}
   do
      cospd="$cospd -1 ${i} -2 ${i%1*}2.unmerged.final.clean.fq -s ${i%1*}u.final.clean.fq "
   done

   cospd_2=" -k auto -o ${ANALYSIS_FOLDER}/coassembly/spades  -t 70 -m 735"

   $cospd_1 $cospd $cospd_2
}

rmdir ${ANALYSIS_FOLDER}/coassembly/megahit/
run_coassem_megahit(){
...
assembly_stats(){
   ...
   #-l megahit,SPAdes \
   #$ANALYSIS_FOLDER/coassembly/megahit/final.contigs.fa \
   #$ANALYSIS_FOLDER/coassembly/spades/contigs.fasta
...
filter_on_assembly_length(){
   ...
   #megahit
   #${TOOLS_FOLDER}/bbmap/reformat.sh \
   #in=${ANALYSIS_FOLDER}/coassembly/megahit/final.contigs.fa \
   #out=${ANALYSIS_FOLDER}/coassembly/megahit/final_1000_filtered.fasta \
   #minlength=1000
